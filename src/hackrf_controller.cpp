...
const int FFT_BIN_WIDTH = 10000;
const uint16_t SWEEP_FREQ_MIN_MHZ = 2400;
const uint16_t SWEEP_FREQ_MAX_MHZ = 2500;
...
extern "C" {
    int hackrf_sweep_fft_callback(void *sweep_state, uint64_t /*frequency*/, hackrf_transfer * /*transfer*/) {
        if (global_controller) {
            ...
            global_controller->handle_fft(fft.size, fft.window, fft.pwr);
        }
        return 0;
    }
}
...
void HackRF_Controller::handle_fft(int size, float *window_ptr, float *pwr_ptr) {
    std::vector<double> x_data(size), y_data(size);
    for (int i = 0; i < size; ++i) {
        x_data[i] = window_ptr[i];
        y_data[i] = pwr_ptr[i];
    }
    if (fft_callback_) {
